...
class CollectClinicals extends React.Component{
    ...
    componentWillMount(){
        console.log("http://localhost:8081/clinicalservices/api/patients/"+this.props.match.params.patientId);
        axios.get("http://localhost:8081/clinicalservices/api/patients/"+this.props.match.params.patientId)
        .then(res=>{
    ...
    handleSubmit(event){
        ...
        axios.post("http://localhost:8081/clinicalservices/api/clinicals",data)
        .then(res=>{
